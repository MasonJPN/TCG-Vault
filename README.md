...
TCG Vault is built around the TCGdex API, an open Pokémon TCG database that provides real card data including artwork, names, card numbers, and move descriptions. The homepage displays the full Scarlet & Violet series as a set grid — clicking any set fetches that set's complete card list from the API and renders it as a paginated card gallery.
